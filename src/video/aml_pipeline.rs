...
fn resolve_capture_params(
    ...
    match hdr_mode {
        ...
        crate::config::HdrMode::SdrOnly => {
            let color_mode = match signal_info.hdr_status {
...
fn open_capture_for_resolution(
    ...
    let is_hdr = output_fmt == VfmcapOutputFmt::P010;
    let img_format = if is_hdr {
        VlImgFormat::P010
    } else {
        VlImgFormat::Nv12
    };
    ...
    info!(
        "AML pipeline: source={}x{} max={}x{} target={}x{}",
        source_w,
        source_h,
        config.max_width,
        config.max_height,
        if target_w > 0 { target_w } else { source_w },
        if target_h > 0 { target_h } else { source_h }
    );
    ...
    let actual_resolution = capture.resolution();
...
impl AmlPipeline {
    ...
    fn run_capture_encode_loop(self: Arc<Self>) {
        ...
        'outer: while self.running_flag.load(Ordering::Acquire) {
            ...
            while self.running_flag.load(Ordering::Acquire) {
                ...
                if frame_count == 0 {
                    if let FrameData::DmaBuf(dma) = &capture_result.frame {
                        info!(
                            "AML pipeline: first frame dma fd={} fd2={} w={} h={} bpl={} size={} fmt={:#x}",
            ...
            // Tear down capture
            std::mem::drop(capture);
            info!("AML pipeline: capture closed (Vulkan resources released)");

            // Wait for stable signal before recreating
            ...
            // The outer loop will recreate capture and encoder
            info!(
        ...
        self.running_flag.store(false, Ordering::Release);
        self.thread_done.store(true, Ordering::Release);
        let _ = self.running.send(false);
...
fn wait_for_stable_signal_sysfs(
    ...
    const STABILITY_CHECK_INTERVAL_MS: u64 = 2000;
    ...
    info!(
        "AML pipeline: waiting for stable HDMI signal via sysfs (up to {}s, need {} identical readings @ {}s intervals)",
        MAX_WAIT_SECS, STABILITY_CHECK_COUNT,
        STABILITY_CHECK_INTERVAL_MS / 1000
    );
        ...
        match sysfs_read_resolution() {
            Some((w, h)) => {
                ...
                if stable_count >= STABILITY_CHECK_COUNT {
                    let tw = if config.max_width > 0 && last_w > config.max_width {
                        config.max_width
                    } else {
                        last_w
                    };
                    let th = if config.max_height > 0 && last_h > config.max_height {
                        config.max_height
                    } else {
                        last_h
                    };
